...
interface RendererInterface extends LifeCycle {
...
type FunctionRenderer = (params: Params) => Node | string;
type ClassRenderer = new (params: Params) => RendererInterface;
type StringRenderer = string;

function isFunctionRenderer(
...
function isClassRenderer(
  renderer: FunctionRenderer | ClassRenderer | StringRenderer
): renderer is ClassRenderer {
  return typeof renderer === 'function' && 'init' in renderer;
}

function parse(renderer: Renderer): Renderer {
  const wrapper = document.createElement('div');
  wrapper.className = 'vanilla-context-li-wrapper';
